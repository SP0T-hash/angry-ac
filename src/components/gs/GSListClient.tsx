...
import {
  Building2, Users, FileText, Wallet, Ticket, Scissors, LogOut, LayoutDashboard, Plus,
} from 'lucide-react';
...
export default function GSListClient({
  usuario, nivelLabel, isAR, titulo, subtitulo, tabela, colunas, linhas, campos, erro,
}: GSListClientProps) {
    ...
    <div className="min-h-screen flex bg-[var(--color-surface)]">
      <aside className="hidden md:flex w-[68px] bg-white/70 backdrop-blur-lg border-r border-slate-100 h-screen flex-col items-center py-4 shrink-0 z-40">
        ...
        <nav className="flex-1 space-y-2 w-full px-2">
          {nav.map((item) => (
            <button key={item.id} title={item.label} onClick={() => router.push(item.href)}
        ...
        <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
          <div className="mb-6 flex items-end justify-between">
            ...
            <button onClick={openNovo}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition active:scale-95 shadow-lg shadow-emerald-600/20">
              <Plus size={14} /> Novo
            </button>
          </div>
